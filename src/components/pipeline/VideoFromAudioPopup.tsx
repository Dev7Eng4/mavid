...
export function VideoFromAudioPopup({ onConfirm, onCancel }: Props) {
  ...
  function handleSubmit() {
    onConfirm({ mode, channel, background, stockVideoCount, audioSpeed, showLogo });
  }
      ...
      >
        ...
        <div className='px-5 py-4 space-y-4'>
          ...
          <div className='space-y-2'>
            ...
            <div className='flex rounded-lg p-0.5' style={{ background: 'var(--code-bg)', border: '1px solid var(--border)' }}>
              {(['batch', 'single'] as const).map(m => (
                <button
                  key={m}
                  onClick={() => setMode(m)}
                  className='flex-1 rounded-md px-3 py-[7px] text-xs font-medium transition-all duration-150 cursor-pointer'
                  style={{
                ...
                >
                  {m === 'batch' ? 'Batch (hàng loạt)' : 'Single (đơn lẻ)'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className='space-y-2'>
            ...
            <CustomSelect
              ...
              emptyText='Không tìm thấy folder backgrounds'
            />
          </div>
