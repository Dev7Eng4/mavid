...
export function PageHeader({ title, description, actions, align = 'center', className = '' }: PageHeaderProps) {
  ...
  return (
    <div
      className={`flex flex-col gap-3 min-w-0 sm:flex-row ${rowAlign} sm:justify-between sm:gap-4 ${className}`.trim()}
    >
      <div className='min-w-0 flex-1 space-y-2'>
        ...
        {description != null && description !== false ? (
          typeof description === 'string' ? (
            ...
            <div className='text-sm leading-relaxed' style={{ color: 'var(--text)' }}>{description}</div>
          )
        ) : null}
      </div>
      {actions != null && actions !== false ? (
        <div className='flex flex-wrap items-center gap-2 shrink-0 w-full sm:w-auto justify-end'>{actions}</div>
      ) : null}
    </div>
  );
}
