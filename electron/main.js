...
const CONSTANTS_APP_SETTINGS_FILE = path.join(CONSTANTS_DIR, 'appSettings.js');
...
/** Hằng chỉnh trong Settings UI — ghi `contents/constants/appSettings.js`. */
const APP_SETTINGS_EXPORT_KEYS = [
  'flowSettings',
  'GEMINI_CONFIG',
  'GEMINI_CHUNK_SIZE',
  'STOCK_VIDEO',
...
/** Hằng còn lại trong `contents/constants/index.js` (không nằm trong appSettings). */
const INDEX_ONLY_EXPORT_KEYS = [
  'MAKE_VIDEO_MODE',
  'VIDEO_TYPE',
...
const CONSTANT_EXPORT_KEYS = [...APP_SETTINGS_EXPORT_KEYS, ...INDEX_ONLY_EXPORT_KEYS];
...
/** Gộp overlay JSON lên module đã import (chỉ các key Settings). */
function mergeConstantsBaseWithUserOverlay(base, overlay) {
...
/**
 * Giá trị mặc định khi chưa có `appSettings.js` (import `index.js` lỗi do re-export thiếu file).
...
async function importConstantsFresh() {
  ...
  try {
    ...
    console.warn(
      '[MaVid] Không import được contents/constants/index.js (thường do thiếu appSettings.js). Dùng giá trị mặc định cho đến khi Lưu settings.',
      e instanceof Error ? e.message : e,
    );
  ...
  if (app.isPackaged) {
    ...
    if (overlay) mod = mergeConstantsBaseWithUserOverlay(mod, overlay);
  }
  return mod;
}
...
async function writeConstantsFiles(nextValues) {
  ...
  const appHeader = `// Auto-generated by MaVid UI (edit settings)\n// File path: contents/constants/appSettings.js\n// Các hằng được chỉnh trong Settings (Flow, Gemini, lưu trữ, stock, phụ đề, logo).\n\n`;
  const appBody = APP_SETTINGS_EXPORT_KEYS.map(name => {
    const v = nextValues[name];
    if (v === undefined) throw new Error(`Thiếu giá trị export app settings: ${name}`);
    return `export const ${name} = ${JSON.stringify(v, null, 2)};`;
  }).join('\n\n');
  fs.writeFileSync(CONSTANTS_APP_SETTINGS_FILE, appHeader + appBody + '\n', 'utf-8');

  const indexHeader = `// Auto-generated by MaVid UI (edit settings)\n// File path: contents/constants/index.js\n// Hằng chỉnh trong Settings nằm trong appSettings.js (re-export bên dưới).\n\n`;
  const reexportNames = APP_SETTINGS_EXPORT_KEYS.join(',\n  ');
  const reexport = `export {\n  ${reexportNames}\n} from './appSettings.js';\n\n`;
  const indexBody = INDEX_ONLY_EXPORT_KEYS.map(name => {
    const v = nextValues[name];
    if (v === undefined) throw new Error(`Thiếu giá trị export index: ${name}`);
    return `export const ${name} = ${JSON.stringify(v, null, 2)};`;
  }).join('\n\n');
  fs.writeFileSync(CONSTANTS_INDEX_FILE, indexHeader + reexport + indexBody + '\n', 'utf-8');
}
