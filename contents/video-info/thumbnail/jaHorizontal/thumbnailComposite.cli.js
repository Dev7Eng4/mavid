...
const HTML_PATH = path.join(__dirname, 'thumbnailHorizontalFlowComposite.html');
...
/**
 * Ghép ảnh Flow (nền) + chữ từ payload bước visual (thumbnail_copy, layout_tokens, …).
 *
 * @param {object} opts
 * @param {string} opts.backgroundImagePath — ảnh JPG từ Flow (vd. flow-thumbnail.jpg)
 * @param {Record<string, unknown>} opts.flowLayout — object JSON từ bước 3 Gemini (có thumbnail_copy, layout_tokens, …)
 * @param {string} opts.outPath — file đích (.jpg/.jpeg → sharp; .png → PNG)
 */
export async function renderThumbnailHorizontalFlowCompositeToPath({ backgroundImagePath, flowLayout, outPath }) {
  ...
  try {
    ...
    console.log('Đã ghép thumbnail horizontal:', abs);
  } finally {
    await browser.close();
  }
}
