...
/**
 * Chỉ `1`, `2`, `1-2` là hợp lệ; chuỗi/ số khác (vd. `3`) được coi như `1`.
 * @param {string} raw
 * @returns {'1' | '2' | '1-2'}
 */
...
export function readMavidChannelConfigFile(channelFolder) {
  ...
  const p = path.join(resolveChannelsDir(), safe, CONFIG_NAME);
  if (!fs.existsSync(p)) throw new Error(`Không tìm thấy ${CONFIG_NAME} trong thư mục kênh «${safe}».`);
  let data;
  try {
    data = JSON.parse(fs.readFileSync(p, 'utf-8'));
  } catch {
    throw new Error(`File ${CONFIG_NAME} không đọc được (JSON hỏng).`);
  }
...
export function getYoutubePublishPlan({ channelFolder, email, uploadCount }) {
  ...
  const config = readMavidChannelConfigFile(channelFolder);
